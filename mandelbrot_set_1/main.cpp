...
RenderWindow window(VideoMode({(unsigned int)WIDTH, (unsigned int)HEIGHT}), "Mandelbrot Set Visualization");
...
Color render_pixel(const float& x, const float& y) {
    Vector2f C = {(4.0f * x / WIDTH - 2.0f) * ratio, 4.0f * y / HEIGHT - 2.0f},
             Z = {0.0, 0.0},
             old_Z;
...
void update_screen() {
    for (int y = 0; y < (int)HEIGHT; ++y) {
        for (int x = 0; x < (int)WIDTH; ++x) {
            Color col = render_pixel((float)x, (float)y);
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (clock.getElapsedTime().asMilliseconds() >= 1000) {
            window.setTitle("Mandelbrot Set Visualization (FPS: " + to_string(frames / ((float)clock.restart().asMilliseconds() / 1000.0)) + ')');
            frames = 0.0;
        }
    }
